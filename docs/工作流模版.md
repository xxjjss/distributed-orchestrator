...
> **背压在飞计数的 crash-leak 回收（V4 补，与 tech-design §2.1 一致）**：上一条 V3 note 提出"按 submitter 分片的在飞计数（认领 `ADD +1`、完成/失败 `ADD -1`）"存在一个**三阶正确性缺口**——若 worker 在 `+1` 之后、`-1` 之前 **crash**（而 crash/关机正是本系统存在的理由），该 shard 计数**只增不减泄漏**，多次 crash 后背压阈值被幽灵计数**永久虚高（假背压）**、拒绝认领健康任务。根因：独立计数器不是从记录派生的真相，而是可独立漂移的旁路状态——与 CAS lease（正确性靠"记录存在性 + leaseExpiry"这个真相支撑，天然 crash-safe）本质不同。
>
> **V4 决策：弃用独立在飞计数器，改由守护层从 `GSI2(affinity+progress)` 直接 `Count`（`Select=COUNT`）活跃 `in-progress` 记录派生背压真值。** 真相从记录派生 → 天然 crash-safe：crash 的任务其 lease 会过期，被守护层按 §4「执行超时」重新计入/回收，派生计数自动收敛。代价是一次 GSI Count 查询（不拉回记录体，成本 = 扫描到的记录数 × 0.5 RCU，可按 submitter 分片 Query 降量）。**保留计数器为备选**：若压测显示 GSI Count 频率过高，可保留原子计数器但叠加 lease 过期驱动的周期性 reconcile（守护层重扫时用 GSI Count 重算 shard 真值、覆盖漂移的计数器），把计数器降级为"缓存"、GSI Count 为"事实源"；一期默认走派生真值（无缓存一致性负担）。
>
> **背压是软上限、非硬保证**：无论派生 Count 还是分片计数，多实例并发认领下背压阈值判定都是**最终一致读**，存在窗口误差（可能瞬时略超上限后收敛）。一期明确把背压定位为「防雪崩的软阀门」，而非精确闸门；需要硬上限处（如 per-workspace token 预算）用**单项记录上的 CAS 硬约束**（强一致），不依赖聚合计数。
...
## 数据安全与持久化写入约束（V5，2026-08-12，回应 PR #4 human comment HC-2）

StateStore 的持久化不仅是"能读能写"，还须满足数据安全与鉴权约束——这些约束直接影响状态机的恢复/幂等实现，故落在本模版：

- **写入鉴权（谁能写 StateStore）**：
  - **人的身份（本机开发者，一期主路径）** = 短时 JIT 凭据（内部 PCSK 系统：Yubikey 登录 + 审批 + 短时 CLI 凭据，time-boxed）。**凭据过期即告警、不硬跑**——与 D2 的 MCP OAuth 过期"只告警不自弹登录"同一原则：无头 worker 遇到写凭据过期时，把 workspace 停在可恢复点（`waiting`）+ 回叫通知人一键续期，而非用失效凭据反复失败写、污染 `attemptCount`。
  - **服务身份（云端引擎/worker，二期）** = IAM Role/instance profile（无存储密钥）；外部系统（GUS）走服务账号 + Vault key。
- **数据安全（防人为误删/越权改状态）**：
  - **表级 `prevent_destroy`**：Terraform `lifecycle { prevent_destroy = true }`，避免误删整表（TCM RCA 教训：`falcon addons tidy --all` 曾误删 tier-1 表致 outage）。
  - **每工作负载最小权限**：worker 只被授予读写自己 `submitter` 分区的权限（呼应 R6 per-submitter 隔离）。
  - **append-only 软删除**：状态跃迁只追加 checkpoint、终态用 `progress=error/done` 标记，**不物理删除历史记录**——这既是审计轨迹（§2c 反馈闭环数据源），也保证恢复算法始终能读到完整的最新有效记录 + 历史。**人类不得直接 delete/update 状态数据项**（只能通过编排器的 `transition`/`fail` 等受控入口），越权直改会破坏 lease/CAS 与 memoization 的一致性。
  - **CloudTrail 归因**：所有写入可回溯到具体身份，用于事后审计。
- **provision 路径对持久化实现的影响**：本地 SQLite 起步（单机 dogfood，最简）→ Falcon addon 提供的 DynamoDB（经管线，推荐）→ adhoc（不推荐）。StateStore 接口抽象须同时支持 SQLite 与 DynamoDB 后端，故恢复/幂等算法不得依赖 DynamoDB 特有语义之外的东西（CAS 用条件写、GSI 派生背压真值在 SQLite 侧用等价索引查询替代）。

> 置信度：PCSK 全称与 Falcon addon provision 的确切 Terraform 结构本轮无法用内部 codesearch/企业搜索核实（鉴权不可用），以上为基于作者已掌握资料的工作假设，立项后须向平台/合规团队核实（见 tech-design §1e 来源与置信度声明）。
