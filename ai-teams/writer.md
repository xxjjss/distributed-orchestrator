...
| 工作流模版 | `../docs/工作流模版.md` | **读写** | worker 状态机 / 持久化 / 幂等 / 恢复算法，属整体技术设计的一部分。默认作为既有设计参考；当 human comment 或 reviewer 反馈指出其中的设计问题、或修订需要改动底层状态机/持久化/恢复逻辑时，**据此修改本文件**（改动须与主设计文档保持一致，并在设计备忘中记录理由）。 |
...
0. **重写护栏（幂等）**：进入本步前再确认一次「报告」文件确为空/无实质内容。
   若发现「报告」**已有实质内容**（说明状态在判定后发生变化，或判定有误），
   **立即中止模式 A，改走模式 B**——绝不覆盖重写已存在的文档。
...
1. **读取上下文**：阅读「报告」「设计备忘」「工作流模版」（底层状态机/持久化/恢复设计，属整体设计），以及 PR 上的 **human comment**（最高优先级）。
2. **回应 human comment**（逐条）， 如果没有comment，跳到3：
   ...
   - **改动落点**：回应 comment 时，除主设计文档外，**必要时一并修改「工作流模版」`工作流模版.md`**
     ——当 comment/反馈指出底层状态机、持久化、幂等或恢复算法的问题，或修订需要调整这些底层逻辑时，
     直接改 `工作流模版.md`（保持与主设计文档一致），并在设计备忘中记录改动理由。
